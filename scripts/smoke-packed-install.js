...
function main() {
  ...
  try {
    ...
    const installedCliPath = path.join(installedPackageDir, 'video-cli.js');
    ...
    assert.ok(fs.existsSync(installedCliPath), 'installed cli entrypoint is missing');
    assert.ok(fs.existsSync(installedBinPath), 'installed cli bin wrapper is missing');
    assert.ok(fs.existsSync(path.join(installedPackageDir, 'skills', 'video-cli', 'SKILL.md')), 'bundled skill is missing');
    assert.ok(fs.existsSync(path.join(installedPackageDir, 'SKILL.md')), 'root skill doc is missing');
    assert.ok(fs.existsSync(path.join(installedPackageDir, 'assets', 'logo.png')), 'packaged logo asset is missing');

    const help = runCommand(process.execPath, [installedCliPath, '--help'], {
      cwd: installDir,
    ...
    const config = JSON.parse(runCommand(process.execPath, [installedCliPath, 'config'], {
      cwd: installDir,
      env: smokeEnv(installDir),
    }).stdout);
